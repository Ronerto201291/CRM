...
public sealed class FacturaEService : IFacturaEService
{
    ...
    private static readonly XNamespace FE = FacturaEConstants.Namespace;
    private static readonly XNamespace DS = "http://www.w3.org/2000/09/xmldsig#";
    private static readonly XNamespace VerifactuRg =
        FacturaEConstants.VerifactuExtensionNamespace;

    private readonly IBillingDbContext _billing;
    private readonly IApplicationDbContext _app;
    private readonly FacturaESigningService _signer;

    public FacturaEService(
        IBillingDbContext billing,
        IApplicationDbContext app,
        FacturaESigningService signer)
    {
        _billing = billing;
        _app     = app;
        _signer  = signer;
    ...
    private static XDocument BuildFacturaE(Invoice invoice, Company? company)
    {
        ...
        return new XDocument(
            ...
            new XElement(FE + "Facturae",
                ...
                new XElement(FE + "FileHeader",
                    new XElement(FE + "SchemaVersion", FacturaEConstants.SchemaVersion),
                    new XElement(FE + "Modality", "I"),
                    new XElement(FE + "InvoiceIssuerType", "EM"),
                    new XElement(FE + "Batch",
                        new XElement(FE + "BatchIdentifier", $"{sellerNif}{invoice.Number}"),
                ...
                new XElement(FE + "Parties",
                    PartyElement("SellerParty", sellerNif, sellerName,
                        sellerAddress, company?.Country ?? "ES"),
                    PartyElement("BuyerParty", buyerNif, buyerName, buyerAddress)),
                        ...
                        invoice.IrpfAmount > 0
                            ? new XElement(FE + "TaxesWithheld",
                                new XElement(FE + "Tax",
                                    ...
                                    new XElement(FE + "TaxRate", Fmt(invoice.IrpfRate)),
                                    new XElement(FE + "TaxableBase",
                                        new XElement(FE + "TotalAmount", Fmt(invoice.Subtotal))),
                                    new XElement(FE + "TaxAmount",
                                        new XElement(FE + "TotalAmount", Fmt(invoice.IrpfAmount)))))
                            : null,

                        new XElement(FE + "InvoiceTotals",
                            new XElement(FE + "TotalGrossAmount", Fmt(invoice.Subtotal)),
                            new XElement(FE + "TotalGrossAmountBeforeTaxes", Fmt(invoice.Subtotal)),
                            new XElement(FE + "TotalTaxOutputs", Fmt(invoice.TaxAmount)),
                            new XElement(FE + "TotalTaxesWithheld", Fmt(invoice.IrpfAmount)),
                            new XElement(FE + "InvoiceTotal", Fmt(invoice.Total)),
                            new XElement(FE + "TotalOutstandingAmount", Fmt(invoice.Total)),
                            new XElement(FE + "TotalExecutableAmount", Fmt(invoice.Total))),

                        new XElement(FE + "Items",
                            invoice.InvoiceLines.Select(line =>
                                new XElement(FE + "InvoiceLine",
                                    new XElement(FE + "ItemDescription", line.Description),
                                    new XElement(FE + "Quantity", FmtQty(line.Quantity)),
                                    new XElement(FE + "UnitOfMeasure", "01"),
                                    new XElement(FE + "UnitPriceWithoutTax", Fmt(line.UnitPrice)),
                                    new XElement(FE + "TotalCost", Fmt(line.LineTotal)),
                                    new XElement(FE + "GrossAmount", Fmt(line.LineTotal)),
                                    line.TaxRate > 0
                                        ? new XElement(FE + "TaxesOutputs",
                                            new XElement(FE + "Tax",
                                                new XElement(FE + "TaxTypeCode", "01"),
                                                new XElement(FE + "TaxRate", Fmt(line.TaxRate)),
                                                new XElement(FE + "TaxableBase",
                                                    new XElement(FE + "TotalAmount", Fmt(line.LineTotal))),
                                                new XElement(FE + "TaxAmount",
                                                    new XElement(FE + "TotalAmount", Fmt(line.TaxAmount)))))
                                        : null))),
    ...
    private static XElement RepresentacionGraficaExtension(
        Invoice invoice, Func<decimal, string> fmtDec)
    {
        ...
        sb.Append(fmtDec(invoice.Total));

        return new XElement(FE + "Extensions",
            new XElement(FE + "Extension",
                new XElement(FE + "ExtensionContent",
                    new XElement(VerifactuRg + "RepresentacionGrafica",
                        new XAttribute(XNamespace.Xmlns + "vrg", VerifactuRg.NamespaceName),
                        new XElement(VerifactuRg + "Version", "1.0"),
                        new XElement(VerifactuRg + "Payload", sb.ToString())))));
    }

    private static XElement PartyElement(
        string tag, string nif, string name, string address, string country = "ES")
    {
        ...
        XElement addressEl = string.Equals(country, "ES", StringComparison.OrdinalIgnoreCase)
            ? new XElement(FE + "AddressInSpain",
                new XElement(FE + "Address", address.Length > 80 ? address[..80] : address),
                new XElement(FE + "PostCode", postCode),
                new XElement(FE + "Town", town),
                new XElement(FE + "Province", town),
                new XElement(FE + "CountryCode", "ESP"))
            : new XElement(FE + "OverseasAddress",
                new XElement(FE + "Address", address.Length > 80 ? address[..80] : address),
                new XElement(FE + "PostCodeAndTown", "N/D"),
                new XElement(FE + "Province", "N/D"),
                new XElement(FE + "CountryCode", countryCode));

        var entityEl = personType == "J"
            ? new XElement(FE + "LegalEntity",
                new XElement(FE + "CorporateName", name.Length > 80 ? name[..80] : name),
                addressEl)
            : new XElement(FE + "Individual",
                new XElement(FE + "Name", name.Length > 40 ? name[..40] : name),
                new XElement(FE + "FirstSurname", "N/D"),
                addressEl);

        return new XElement(FE + tag,
            new XElement(FE + "TaxIdentification",
                new XElement(FE + "PersonTypeCode", personType),
                new XElement(FE + "ResidenceTypeCode", residence),
                new XElement(FE + "TaxIdentificationNumber", nif)),
            entityEl);
    }
    ...
    private static string CountryCodeToIso3(string iso2) => iso2.ToUpperInvariant() switch
    {
        ...
        var x when x.Length == 3 => x,
        var x => x.PadRight(3, 'X')
    };
