    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                var crmBridge  = scope.ServiceProvider.GetRequiredService<IExpenseOcrCrmBridge>();
                ...
                foreach (var upload in pending)
                {
                    ...
                    try
                    {
                        ...
                        {
                            var supplier = await crmBridge.FindOrCreateSupplierAsync(
                                upload.CompanyId, doc.SupplierTaxId, doc.SupplierName, stoppingToken);
                            if (supplier is not null)
                            {
                                doc.SupplierId = supplier.SupplierId;
                        ...
                        await crmBridge.LogOcrActivityAsync(
                            upload.CompanyId, "ExpenseDocument", doc.Id, "OcrProcessed",
                            $"OCR completado (confianza {confidence:F1}%, {lines.Count} líneas): Base={doc.TaxBase}, IVA={doc.VATAmount}, Total={doc.Total}",
                            stoppingToken);
