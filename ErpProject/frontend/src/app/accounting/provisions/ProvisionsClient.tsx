...
export default function ProvisionsClient({
    initialProvisions,
  ...
  const handleRelease = async (id: string) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al liberar la provisión');
    } finally {
  ...
  return (
    <PageContainer>
      ...
      <AccessibleModal open={showModal} onClose={() => setShowModal(false)} title="Nueva Provisión Contable" maxWidth="520px">
      ...
      <AccessibleModal open={!!releaseId} onClose={() => setReleaseId(null)} title="Liberar Provisión" maxWidth="420px"
        footer={(<div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
          ...
          <button className="btn-primary" style={{ background: '#7c3aed', borderColor: '#7c3aed' }}
            disabled={releaseLoading} onClick={() => releaseId && handleRelease(releaseId)}>
