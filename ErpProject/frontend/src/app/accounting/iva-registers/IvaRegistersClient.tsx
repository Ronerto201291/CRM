...
import type { InvoiceRow, ExpenseDoc, RegistersSummary } from "./page";

interface IvaRegistersClientProps {
  initialYear: number;
  initialRegisters: RegistersSummary;
  initialPurchaseRows: ExpenseDoc[];
  initialSalesRows: InvoiceRow[];
}

const fmt = (n: number) =>
  `€${n.toLocaleString("es-ES", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`;

export default function IvaRegistersClient({
  initialYear,
  initialRegisters,
  initialPurchaseRows,
  initialSalesRows,
}: IvaRegistersClientProps) {
  const [year, setYear] = useState(initialYear);
  const [registers, setRegisters] = useState<RegistersSummary>(initialRegisters);
  const [purchaseRows, setPurchaseRows] = useState<ExpenseDoc[]>(initialPurchaseRows);
  const [salesRows, setSalesRows] = useState<InvoiceRow[]>(initialSalesRows);
  ...
  const load = useCallback(async (forYear: number) => {
    ...
    try {
      ...
      const yearInvoices = invoices.filter((i) => {
        ...
        return d ? new Date(d).getFullYear() === forYear : false;
      });
