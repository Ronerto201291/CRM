    ...
    const handleAdjust = async () => {
        ...
        const r = await fetch('/api/proxy/inventory/stock/adjustment', {
            ...
            body: JSON.stringify({
                ...
                quantity: adjForm.movementType === 'StockOut' ? -Math.abs(adjForm.quantity) : Math.abs(adjForm.quantity),
                ...
                <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setShowModal(false); }}>
                    <div className="modal-box" style={{ maxWidth: '460px' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '20px' }}>
                            <div className="form-group" style={{ gridColumn: 'span 2' }}><label className="erp-label">NOMBRE *</label><input className="erp-input" placeholder="Nombre del producto" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
                            <div className="form-group"><label className="erp-label">SKU / REFERENCIA</label><input className="erp-input" placeholder="REF-001" value={form.sku} onChange={e => setForm({ ...form, sku: e.target.value })} /></div>
                            <div className="form-group"><label className="erp-label">PRECIO UNITARIO (€)</label><input type="number" className="erp-input" min="0" step="0.01" value={form.price} onChange={e => setForm({ ...form, price: +e.target.value })} /></div>
