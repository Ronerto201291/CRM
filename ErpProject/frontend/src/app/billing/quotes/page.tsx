...
export default function QuotesPage() {
    ...
    // ── Line helpers ──────────────────────────────────────────────────────────
    const updateLine = (i: number, field: keyof LineForm, val: any) => {
    ...
    const handleCreate = async () => {
        if (clientType === 'Registered' && !form.clientId) { alert('Selecciona un cliente'); return; }
        if (clientType === 'Lead' && !form.clientId) { alert('Selecciona un posible cliente'); return; }
        if (clientType === 'Manual' && !form.clientName) { alert('Introduce el nombre del cliente'); return; }
        if (form.lines.every(l => !l.description)) { alert('Añade al menos una línea'); return; }
        setSaving(true);
        try {
            const body = {
                clientType,
            ...
            if (res.ok) {
                ...
                load();
            } else { const e = await res.json(); alert(e.error || 'Error al crear presupuesto'); }
    ...
    const handleSend = async () => {
        if (!sendForm.toEmail) { alert('Introduce el email'); return; }
        setSending(true);
        try {
            ...
            if (res.ok) { setShowSend(false); load(); }
            else { const e = await res.json(); alert(e.error || 'Error al enviar'); }
    ...
    const handleAccept = async (id: string) => {
        ...
        if (res.ok) load(); else { const e = await res.json(); alert(e.error || 'Error'); }
    };
    ...
    const handleReject = async () => {
        ...
        if (res.ok) { setShowReject(false); load(); }
    ...
    const handleConvert = async () => {
        ...
        if (res.ok) { setShowConvert(false); load(); alert('✅ Presupuesto convertido a factura'); }
        else { const e = await res.json(); alert(e.error || 'Error al convertir'); }
    };
    ...
    return (
        <PageContainer>
                ...
                <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setShowCreate(false); }}>
                    <div className="modal-box" style={{ maxWidth: '780px', maxHeight: '90vh', overflowY: 'auto' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                            <h2 style={{ fontSize: '18px', fontWeight: 700 }}>Nuevo Presupuesto</h2>
                            <button onClick={() => setShowCreate(false)} style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: '20px', color: 'var(--text-muted)' }}>✕</button>
                        </div>
            ...
            {/* ─── Send Modal ───────────────────────────────────────────────────── */}
            {showSend && (
                <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setShowSend(false); }}>
                    <div className="modal-box" style={{ maxWidth: '440px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                            <h2 style={{ fontSize: '17px', fontWeight: 700 }}>📧 Enviar Presupuesto</h2>
                            <button onClick={() => setShowSend(false)} style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: '20px', color: 'var(--text-muted)' }}>✕</button>
                        </div>
            ...
            {/* ─── Reject Modal ─────────────────────────────────────────────────── */}
            {showReject && (
                <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setShowReject(false); }}>
                    <div className="modal-box" style={{ maxWidth: '420px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                            <h2 style={{ fontSize: '17px', fontWeight: 700 }}>Rechazar Presupuesto</h2>
                            <button onClick={() => setShowReject(false)} style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: '20px', color: 'var(--text-muted)' }}>✕</button>
                        </div>
            ...
            {/* ─── Convert Modal ────────────────────────────────────────────────── */}
            {showConvert && (
                <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setShowConvert(false); }}>
                    <div className="modal-box" style={{ maxWidth: '400px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                            <h2 style={{ fontSize: '17px', fontWeight: 700 }}>Convertir a Factura</h2>
                            <button onClick={() => setShowConvert(false)} style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: '20px', color: 'var(--text-muted)' }}>✕</button>
                        </div>
                        <div style={{ background: 'var(--surface-2)', borderRadius: '8px', padding: '10px 14px', marginBottom: '16px', fontSize: '13px', color: 'var(--text-secondary)' }}>
