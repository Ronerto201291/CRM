...
export default function EmpresasPage() {
    ...
    return (
        <div style={{ padding: '28px 32px', fontFamily: 'Inter, sans-serif' }}>
            ...
            {showModal && (
                <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) { setShowModal(false); setInviteResult(null); } }}>
                    <div className="modal-box" style={{ maxWidth: '480px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                            <h2 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text-primary)' }}>
                                Invitar nueva empresa
                            </h2>
                            <button onClick={() => { setShowModal(false); setInviteResult(null); }} style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: '20px', color: 'var(--text-muted)' }}>✕</button>
                        </div>
