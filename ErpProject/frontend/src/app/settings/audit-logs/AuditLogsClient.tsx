...
export default function AuditLogsClient({ initialLogs, initialDateFrom, initialDateTo }: AuditLogsClientProps) {
    ...
    useEffect(() => {
        ...
        if (filter.dateFrom === initialDateFrom && filter.dateTo === initialDateTo
            && filter.action === 'all' && filter.entityType === 'all'
            && initialLogs.length > 0) {
            return;
        }
        loadLogs();
    }, [filter, initialDateFrom, initialDateTo, initialLogs.length]);
    ...
    return (
        <PageListLayout
            ...
            subtitle="Historial completo de cambios en el sistema (Compliance)"
        >
            {/* Filtros */}
            <div className="erp-card" style={{ padding: '16px 20px', marginBottom: '20px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '16px' }}>
                    <div>
                        <FormLabel htmlFor="audit-filter-action">Acción</FormLabel>
                        <select id="audit-filter-action" className="erp-input" value={filter.action} onChange={(e) => setFilter({ ...filter, action: e.target.value })}>
                    ...
                    <div>
                        <FormLabel htmlFor="audit-filter-entity">Tipo de entidad</FormLabel>
                        <select id="audit-filter-entity" className="erp-input" value={filter.entityType} onChange={(e) => setFilter({ ...filter, entityType: e.target.value })}>
                            <option value="all">Todas</option>
                    ...
                    <div>
                        <FormLabel htmlFor="audit-filter-from">Desde</FormLabel>
                        <input id="audit-filter-from" className="erp-input" type="date" value={filter.dateFrom} onChange={(e) => setFilter({ ...filter, dateFrom: e.target.value })} />
                    </div>
                    <div>
                        <FormLabel htmlFor="audit-filter-to">Hasta</FormLabel>
                        <input id="audit-filter-to" className="erp-input" type="date" value={filter.dateTo} onChange={(e) => setFilter({ ...filter, dateTo: e.target.value })} />
                    </div>
                </div>
            </div>
