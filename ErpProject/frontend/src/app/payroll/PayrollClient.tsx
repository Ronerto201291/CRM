...
export default function PayrollClient({
    ...
    const addEmployee = async () => {
        setFormError(null);
        if (!empForm.taxId || !empForm.fullName) { setFormError('NIF y nombre obligatorios'); return; }
        setLoading(true);
        try {
    ...
    const addLine = async () => {
        setFormError(null);
        if (!lineForm.settlementId || !lineForm.employeeId) { setFormError('Selecciona liquidación y empleado'); return; }
        setLoading(true);
        try {
