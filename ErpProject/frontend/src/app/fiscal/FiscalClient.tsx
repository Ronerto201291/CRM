...
interface FiscalClientProps {
    initialEvents: FiscalEvent[];
    initialYear: number;
}

export default function FiscalClient({ initialEvents, initialYear }: FiscalClientProps) {
    const [events, setEvents] = useState<FiscalEvent[]>(initialEvents);
    ...
    const pending = useMemo(() => events.filter(e => e.status === 'Pending' || e.status === 'Reminded' || e.status === 'InProgress'), [events]);
    const overdue = useMemo(() => events.filter(e => e.isOverdue), [events]);
    const submitted = useMemo(() => events.filter(e => e.status === 'Submitted' || e.status === 'Paid'), [events]);
    const upcoming = useMemo(() => events.filter(e => e.diasRestantes >= 0 && e.diasRestantes <= 30 && e.status !== 'Submitted' && e.status !== 'Paid'), [events]);
    const urgentAlerts = useMemo(() => events.filter(e => e.diasRestantes >= 0 && e.diasRestantes <= 7 && e.status !== 'Submitted' && e.status !== 'Paid'), [events]);
    const sortedFiltered = useMemo(() => [...filtered].sort((a, b) => new Date(a.deadlineDate).getTime() - new Date(b.deadlineDate).getTime()), [filtered]);
    ...
    return (
        <PageContainer>
            ...
            {/* Events table */}
            {loading ? (
                <LoadingPlaceholder message="Cargando calendario fiscal..." />
            ) : filtered.length === 0 ? (
                <EmptyState icon="📅" title="No hay eventos para este filtro." />
            ) : (
