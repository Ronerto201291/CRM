...
import { financingConfirmingSchema, financingFactoringSchema, financingCreditLineSchema } from '@/lib/schemas/treasuryFormSchemas';
...
interface FinancingClientProps {
    initialConfirming: ConfirmingOp[];
    initialFactoring: FactoringOp[];
    initialCreditLines: CreditLine[];
}
...
export default function FinancingClient({
    ...
    const submitConfirming = async () => {
        setFormError(null);
        const parsed = financingConfirmingSchema.safeParse(form);
        if (!parsed.success) {
    ...
    const submitFactoring = async () => {
        setFormError(null);
        const parsed = financingFactoringSchema.safeParse(form);
        if (!parsed.success) {
            setFormError(parsed.error.issues[0]?.message ?? 'Revisa el formulario');
            return;
        }
    ...
    const submitCreditLine = async () => {
        setFormError(null);
        const parsed = financingCreditLineSchema.safeParse(form);
        if (!parsed.success) {
            setFormError(parsed.error.issues[0]?.message ?? 'Revisa el formulario');
            return;
        }
    ...
    const payOp = async (type: 'confirming' | 'factoring', id: string) => {
        ...
        if (res.ok) { setSuccessMsg('Pagado'); load(); }
    ...
    return (
        <PageContainer>
            ...
            <AccessibleModal open={showCreate} onClose={() => setShowCreate(false)}
                title={`Nueva ${tab === 'confirming' ? 'Operación de Confirming' : tab === 'factoring' ? 'Operación de Factoring' : 'Línea de Crédito'}`}
                maxWidth="450px">
                        {formError && (
