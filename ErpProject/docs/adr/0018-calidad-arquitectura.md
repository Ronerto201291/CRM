...
| 0c | **FacturaE** — **✅ Corregido en código** (jul 2026) — namespace oficial `FacturaEConstants.Namespace` (`Versiones/Facturaev3_2_2.xml`); `FacturaEXmlStructureValidator` compara contra la constante oficial y **rechaza el namespace obsoleto**; `Extensions` solo `ExtensionContent` con namespace ajeno Verifactu; firma dedicada `FacturaESigningService` XAdES-**EPES** con `SignaturePolicyIdentifier` (política Facturae v3.1); endpoints `GET .../signed`, `GET .../validate`, `POST .../submit-face` vía MediatR. **Pendiente externo**: homologación FACe en entorno test | Billing | **~95%** — código cerrado; homologación test pendiente |
...
| 0e | **Stripe — sin idempotencia de eventos de webhook** | Core (StripeService) | ✅ **Corregido** (jul 2026) — idempotencia (`StripeWebhookEvents`) + `StripeOptionsValidator` (`IValidateOptions<StripeOptions>`): exige prefijo `sk_test_`/`sk_live_` y lo cruza con `IHostEnvironment` (producción → `sk_live_`, resto → `sk_test_`) |
| 0f | **Sin validación de NIF/CIF/NIE** | CRM/Billing/Core | ✅ **Corregido** (jul 2026) — `SpanishTaxIdValidator` centralizado con CIF corregido (rama dígito para `A/B/E/H`, letra para `K/L/M/N/P/Q/R/S/W`, ambos para el resto). Tests con casos reales (`A39000013` Banco Santander, `B12345674`). Integrado en FluentValidation CRM, registro empresa, `UpdateCompanyHandler` y `FacturaEService.ValidateTaxId` |
...
| 24 | Implementar Modelo 303 (IVA trimestral) real a partir de facturas bloqueadas + `JournalEntry` (472 deducible) | Accounting | ✅ Corregido — `Modelo303Reader`, export CSV/XML; `DeclareModelo330Command` persiste en `VatLiquidations`; frontend `accounting/aeat` con export y «Registrar declaración» |
...
| Críticos fiscales 0a–0f | 6 (#0a, #0c código, #0e, #0f) | 2 (#0b, #0d bloqueo externo) | 0 código | **~92%** (homologación externa pendiente) |
