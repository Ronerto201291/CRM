...
| 42b | ~~Conciliación de ingresos por método de pago (TPV, transferencia, Bizum, caja) — `PaymentMethod` string libre sin restricción, solo distinción binaria efectivo/banco en el asiento~~ 🟡 **Ampliado — mitad hecha**: `PaymentMethods` (`Modules/Billing/Application/Features/Billing/PaymentMethods.cs`) restringe a `cash\|bank\|card\|bizum\|transfer` (guard clause inline en `MarkPaidHandler` — Billing no registra `AddValidatorsFromAssembly`, un validador ahí no se ejecutaría). `PaymentReceivedEventHandler` ahora enruta por método: `cash`→"570", `card`→"5721" (TPV pendiente de liquidar), `bizum`→"5722" (Bizum pendiente de liquidar), `bank`/`transfer`→"572" — ambas cuentas nuevas ya las siembra `SeedChartOfAccountsHandler` (ver #0g). `BankAccount` (Treasury) expone `AccountingAccountCode` en el formulario de alta (`TreasuryClient.tsx`) y en la tabla, así que un usuario ya puede crear una cuenta "TPV Redsys" con código "5721" y conciliarla con `BankReconciliationService` (que ya filtra por prefijo de código, sin cambios) sin que se mezcle con una transferencia normal. 9 tests nuevos (`PaymentReceivedEventHandlerTests` — 5 métodos de pago + idempotencia + cuenta faltante; `MarkPaidHandlerTests` — válidos/inválido/idempotente). **Pendiente**: no existe ninguna entidad de sesión/arqueo de caja (`grep` de `CashSession`/`CashRegister`/`ArqueoCaja`: 0 resultados) — abrir/cerrar caja con esperado vs. contado sigue sin construir; es la mitad de este ítem que falta | Billing ↔ Treasury | Alta — el enrutamiento contable ya es real, el arqueo de caja sigue pendiente |
...
| Producto 38–42f | **~30%** | ADR-0019 + `GET /api/platform/product-roadmap`; Fase 1 #42a ✅; #42f ✅ (servicios contratados) |
...
- **#42b (mitad — conciliación por método de pago):** `PaymentMethods` (Billing) restringe el método de pago a `cash\|bank\|card\|bizum\|transfer`; `PaymentReceivedEventHandler` enruta `card`→"5721", `bizum`→"5722" (sembradas por `SeedChartOfAccountsHandler`, #0g) en vez de tratarlas como banco genérico. `BankAccount` (Treasury) ya expone `AccountingAccountCode` en el alta y la tabla del frontend, así que se puede crear una cuenta "TPV"/"Bizum" con esos códigos y conciliarla con `BankReconciliationService` sin cambios (ya filtra por prefijo). 9 tests nuevos. Pendiente: arqueo de caja (`CashSession`, esperado vs. contado) — no se ha tocado.
...
| Producto 38–42f | 1 (#42f) | 1 (#42a parcial) | 8+ documentados | **~20%** |
