...
- Accounting: `AeatModelsController`, `AgingController`,
  `InversionSujetoActivoController`, `IvaManagementController` ✅ **implementados
  de verdad** (#3c jul 2026) — controllers delgados con `IMediator` y handlers
  reales; `FinancialStatementsController` usa MediatR con handlers honestos
  `NotImplemented`. `AccountingExportController` ✅ (#4);
...
- El hallazgo de mayor volumen histórico fue el bypass de CQRS en controllers
  (✅ cerrado: 43/43 con `IMediator`; los cuatro controllers fiscales mock de
  Accounting fueron implementados con lógica real en #3c jul 2026). Cualquier
  trabajo futuro en Accounting debe mantener handlers reales, no reintroducir
  controllers con datos fijos.
...
| 0c | **FacturaE** — **🟡 Parcial+** — firma XAdES real ya presente (antes `.xsig` sin firmar), NIF validado, direcciones ya no hardcodeadas, `POST .../submit-face`. **Verificado de nuevo, 3 problemas reales que la fila anterior no mencionaba**: (1) el namespace raíz sigue siendo el incorrecto (`Version3.2.2/Facturae32.xsd` en vez de `Versiones/Facturaev3_2_2.xml`); (2) el nuevo `FacturaEXmlStructureValidator` valida el namespace **contra esa misma constante equivocada** — es una validación circular que siempre da "correcto" aunque el namespace esté mal, no protege de nada; (3) el bloque `Extensions` sigue mal formado (hijos con nombres inventados en vez de contenido de namespace ajeno) y la firma es XAdES-BES cuando FACe exige XAdES-EPES con `SignaturePolicyIdentifier`. Pendiente: namespace real, validador no circular, perfil de firma correcto, homologación entorno test | Billing | **Crítica — namespace y validador incorrectos, no solo "bloqueado externo"** |
...
| 0f | **Sin validación de NIF/CIF/NIE** | CRM/Billing/Core | ⚠️ **Parcial, con un bug grave — reabierto**: NIF y NIE están bien (`SpanishTaxIdValidator.ValidateNif`/`ValidateNie`, verificado con cálculo manual). **Pero el CIF tiene la lógica de letra/dígito de control invertida**: las letras A/B/E/H (que legalmente llevan dígito de control) reciben letra, y P/Q/R/S/W (que legalmente llevan letra) reciben dígito. Confirmado con ejemplo real: el CIF de Banco Santander `A39000013` (control real `3`) es **rechazado** por este validador, que espera `C`. El test `SpanishTaxIdValidatorTests.FindValidCif` da falsa confianza porque fuerza por fuerza bruta un CIF que el propio validador (ya roto) acepta, así que nunca puede detectar la inversión. Impacto: bloquea el alta de clientes/proveedores/empresas reales tipo S.A./S.L. mientras acepta CIFs fabricados que no lo son. **Prioridad alta de corrección** — esto activamente rompe altas legítimas, no es solo una ausencia de validación |
...
| 3c | `AeatModelsController`, `IvaManagementController`, `InversionSujetoActivoController`, `FinancialStatementsController`, `AgingController` sin `IMediator` | Accounting | ✅ Corregido jul 2026 — los 4 controllers mock ahora son delgados (`IMediator` únicamente). Handlers: `Application/Features/Aeat/`, `IvaManagement/`, `Isp/`, `Aging/`. Servicios cross-módulo: `Infrastructure/Services/{AeatModelsDataService,IvaRegisterDataService,AgingDataService}.cs` (facturas `IBillingDbContext`, gastos `IExpensesDbContext`, persistencia en entidades `Modelo347`, `IvaRegister`, `InversionDeSujetoActivo`, `AgingReport`, `SiiDeclaration`). `FinancialStatementsController` ya era real vía MediatR (#26) |
...
| 18 | `frontend/accounting/iva-registers/page.tsx` y `aeat-models/page.tsx` — mock en cliente | Accounting (frontend) | ✅ Corregido jul 2026 — conectadas vía `/api/proxy/v1/accounting/iva/*` y `/api/proxy/v1/accounting/aeat/*`; también `aging`, `isp`, `cash-flow`, `vat-regime`, `vies`, `recargo`, `prorrata`, `cost-centers` usan proxy real |
...
| 42c | Cruce entre módulo contratado y permiso de usuario (nota: este ítem es sobre módulos de la plataforma SaaS por empresa/usuario — no confundir con el ítem 42f, que es sobre servicios que la pyme le vende a **sus** clientes) — `TenantModule` (`CompanyId`, `ModuleName`, `IsEnabled`) y el toggle de `settings/subscription` ya son reales y funcionan (no mock, verificado: `PUT /api/proxy/tenant/modules/{id}` real), pero la granularidad es solo a nivel empresa. Falta cruzar esto con roles/permisos por usuario (ADR-0002) para poder decir "el módulo está contratado por la empresa, pero este usuario no tiene acceso" — hoy si el módulo está activado, cualquier usuario de esa empresa con el permiso genérico lo ve | Core (Licensing + Auth) | 🟡 Parcial — ya existe una capa ABAC por usuario (`UserPermission` con grant/deny y expiración, resuelta por `PermissionService` según la membresía/rol activo en `UserCompany`), que es la granularidad por-usuario que pedía este ítem. Pero sigue siendo un mecanismo separado de `ModuleAuthorizationHandler` (que gestiona "módulo contratado por la empresa") — las dos capas no están unificadas en una sola comprobación "contratado pero denegado a este usuario" |
...
**Nota sobre el ítem 3c (cerrado jul 2026):**
Los cuatro controllers (`AeatModelsController`, `IvaManagementController`,
`InversionSujetoActivoController`, `AgingController`) fueron **implementados de
verdad** con CQRS/MediatR. Ya no devuelven literales hardcodeados: agregan
facturas bloqueadas, gastos aprobados y persisten en las entidades de dominio
correspondientes. La presentación telemática real (AEAT/SII) sigue bloqueada
por homologación externa (#0b–#0d); los handlers lo documentan honestamente en
...
|---|---|---|---|
| Accounting | `AeatModelsController`, `IvaManagementController`, `InversionSujetoActivoController`, `AgingController` calculan/declaran modelos fiscales reales | ✅ Corregido (#3c jul 2026): MediatR + servicios reales; persistencia en BD. Homologación AEAT/SII para envío telemático sigue pendiente (#0b–#0d) | ADR-0006, backlog #3c |
