...
| 13 | `Erp.Infrastructure` depende de módulos Application (dirección invertida) | Core | ✅ Corregido jul 2026 — `IVerifactuXmlGenerator` movido a `Erp.Application/Common/Interfaces/`; implementación en `Billing.Infrastructure`; eliminada `ProjectReference` `Erp.Infrastructure`→`Billing.Application`; test arquitectura `ErpInfrastructure_DoesNotReferenceModuleApplicationLayers`. `ErpDbContext` solo referencia `*.Domain` para `Ignore<T>()` |
...
| Producto 38–42f | **~70%** | ADR-0019 + `GET /api/platform/product-roadmap`; #38/#40/#41/#42 ✅; #42a ✅ (fases 1–5); #42b ✅; #42c ✅; #42e/#42f ✅ |
