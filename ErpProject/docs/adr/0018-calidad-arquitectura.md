...
| 13 | `Erp.Infrastructure` depende de 5 módulos (dirección invertida) | Core | 🟡 Parcialmente reabierto (contra-auditoría jul 2026) — Inventory/Crm/Accounting/Expenses ya sin refs a `*.Application` de módulo: SII/automation vía puertos (`ISiiEmitidasInvoiceSource`, `ISiiRecibidasExpenseSource`, `IAutomationBillingQuery`, `IAutomationInventoryQuery`); OCR → `Expenses.Infrastructure`; `IExpenseOcrCrmBridge` en CRM. **Pero `Erp.Infrastructure.csproj` sigue referenciando `Erp.Modules.Billing.Application.csproj`, y es una dependencia real y viva**: `Erp.Infrastructure/Features/Sii/SiiHandlers.cs` importa `Erp.Modules.Billing.Application.Interfaces` e inyecta `IVerifactuXmlGenerator` (de esa capa) en `GetVerifactuXmlHandler`/`SubmitVerifactuHandler`. No es código de hoy (viene de antes de esta sesión), pero contradice la fila anterior de este mismo ADR que la daba por resuelta — no cerrar este ítem como ✅ hasta mover esa lógica fuera de `Erp.Infrastructure` (p. ej. un puerto `IVerifactuXmlGenerator` definido en `Erp.Application`, implementado por `Billing.Infrastructure`, siguiendo el mismo patrón ya usado para SII/OCR/CRM arriba). `ErpDbContext` solo referencia `*.Domain` para `Ignore<T>()` |
...
| 42e | ~~Envío periódico de documentos a gestorías externas — no existe ningún mecanismo de exportación/envío por email hoy~~ **🟡 Parcialmente corregido (contra-auditoría jul 2026 — esta fila estaba desactualizada, la funcionalidad ya se construyó)**: `ExportAccountantPackageCommand`/`AccountantExportHandlers.cs` genera un ZIP real; `AccountantExportJob` está registrado en Hangfire con cron real (`Modules/Accounting/Infrastructure/DependencyInjection.cs`, invocado desde `Program.cs`) — no repite el bug histórico de `RuleEvaluatorJob` (job huérfano sin registrar); `AccountantExportController` despacha vía `IMediator`; frontend `settings/accountant-export/AccountantExportClient.tsx` hace fetch real a `/api/proxy/accountant-export/{settings,export,send}`; envío real por SMTP vía `EmailService`/MailKit. **Pero el ZIP solo contiene el libro de IVA (emitidas/recibidas) en CSV + un `LEEME.txt`** — no incluye PDFs de facturas ni los asientos contables del período, que es lo que esta fila y el roadmap original prometían. Decidir: ampliar el contenido del ZIP, o recortar la promesa del roadmap (ADR-0019 #42e) al alcance real actual | Billing/Accounting | Media — cerrar el alcance de contenido del ZIP |
...
| 32 Tests | ✅ | **878 tests** (578 unit + 152 integration + 129 Vitest + 19 E2E) — corregido en contra-auditoría jul 2026: la fila anterior sumaba mal estos mismos cuatro números y afirmaba "763"; 578+152+129+19=878. Cifras individuales verificadas por grep de `[Fact]`/`[Theory]`/`it(`/`test(`, aproximadas (`Erp.IntegrationTests` usa `[Theory]`+`[MemberData]` que generan un test por endpoint descubierto, así que el total real en ejecución puede variar unos pocos puntos); gates merged **52%**, Accounting **22%**, frontend **43%**; E2E TOTP completo con `E2E_2FA_TOTP_SECRET`; `Phase18HandlerTests`; plan `docs/testing-strategy.md` fase 18 |
...
| # | Hallazgo | Módulo | Prioridad |
|---|---|---|---|
| 66 | **Doble descuento de stock en el ciclo de venta completo**: cuando se genera el albarán (`DeliveryNote`) y después se bloquea la factura fiscal (`CustomerInvoice`→`Billing.Invoice`) del mismo pedido, tanto `DeliveryNoteInventoryHandler` como `InvoiceApprovedInventoryHandler` decrementan el mismo `Stock` para los mismos productos/cantidades — cada uno solo comprueba idempotencia dentro de su propio `ReferenceType` (`"DeliveryNote"` vs `"Invoice"`), sin ninguna protección cruzada. Es el flujo estándar order-to-cash del ERP, no un caso límite | Sales ↔ Inventory ↔ Billing | **Alta — bug de integridad de datos en el flujo principal de venta** |
| 67 | **Purchasing: los formularios de recepción y factura de proveedor no pueden completarse desde la UI real** — `receipts/new` e `invoices/new` no recogen `purchaseOrderLineId`/`purchaseOrderId`/`productId`, campos obligatorios (`Guid` no anulable) en `CreateGoodsReceiptCommand`/`CreateSupplierInvoiceCommand`. Toda petición real desde estas páginas lanza `InvalidOperationException` antes de llegar a la lógica de negocio — el three-way match nunca se puede probar por un usuario real, pese a que el backend que lo implementa (`ThreeWayMatchValidator`, `GoodsReceiptInventoryHandler`) es correcto | Purchasing (frontend) | **Alta — bloquea el flujo completo de compras desde la UI** |
| 68 | **Purchasing: los listados de recepciones y facturas de proveedor siempre están vacíos** — `ReceiptsController`/`InvoicesController` solo exponen `POST` y `GET {id}` (stub); no existe ningún `GET` de listado. El frontend llama a una ruta que no existe (404 silencioso), y `serverFetchList` devuelve `[]` sin mostrar error | Purchasing | Alta |
| 69 | **Purchasing→Accounting: el three-way match nunca genera asiento contable** — cero referencias a Purchasing/`SupplierInvoice`/`GoodsReceipt` en todo `Modules/Accounting`; `CreateSupplierInvoiceHandler` no inyecta `IPublisher` ni ningún servicio de Accounting. Las compras quedan fuera de la contabilidad real | Purchasing ↔ Accounting | Media — requiere decisión de alcance, no solo código |
| 70 | **RLS (Row-Level Security) está apagado por defecto fuera de Docker Compose** — `Postgres:RlsEnabled` no figura en ningún `appsettings*.json`; solo se activa vía `docker-compose.override.yml`/`docker-compose.prod.yml`. `k8s/deployment.yaml` no lo define en absoluto — un despliegue en Kubernetes correría con RLS apagado sin ninguna señal de alarma. Además, `PostgresRlsBootstrapTests` hace `catch { Assert.True(true); }` cuando Docker no está disponible — un no-op silencioso, no una verificación real | Core (multi-tenant) | **Alta — riesgo de fuga de datos entre tenants en despliegues sin Docker Compose** |
| 71 | **`CrmDbContext` es el único DbContext de los 9 módulos sin `AuditSaveChangesInterceptor` registrado** — `Modules/Crm/Infrastructure/DependencyInjection.cs` no llama a `.AddInterceptors(...)` al registrar `CrmDbContext`, a diferencia de Expenses/Purchasing/Sales/Accounting/Payroll/Treasury/Billing/Inventory. Altas/bajas/cambios de `Client`/`Lead`/`Supplier` no dejan `AuditLog` — justo el módulo con más volumen de escritura de datos personales (RGPD) | Crm | Alta |
| 72 | **`PosTerminalsController` (TPV físico) tiene backend completo y cero frontend** — persiste `PosPayment`/`PosTerminal` reales y publica `InvoiceCardPaymentRequestedEvent`, pero ningún archivo bajo `frontend/src/app/treasury` lo referencia (`grep` de "pos-terminal"/"PosTerminal"/"TPV" en frontend: solo un placeholder de texto no funcional en `TreasuryClient.tsx`). No está reflejado en ADR-0012, que ni siquiera cuenta este controller en su lista (dice 6 controllers, hay 7) | Treasury (frontend) | Media — decidir: construir frontend o no contarlo como avance del roadmap #41 |
| 73 | **Test que no verifica comportamiento real**: `Billing/MultiCurrencyAndPosTests.cs` → `CreateInvoiceMultiCurrencyTests.ExchangeRateLookup_ConvertsUsdToEur` solo instancia el propio `FakeExchangeRateLookup` (con `["USD"]=0.92m` hardcodeado en el test) y comprueba `100*0.92=92` — no ejercita `CreateInvoiceHandler` ni `ExchangeRateLookupAdapter`/`ExchangeRateService` reales. No existe ningún test que cree una factura en USD vía el handler real y verifique que `CurrencyCode`/`TotalEur` queden bien poblados | Billing (tests) | Baja — deuda de calidad de test, no de producto |

**Nota de proceso**: los ítems 66, 67, 68 y 70 son los que un cliente real
encontraría en la primera semana de uso comercial — priorizarlos por delante
de cualquier trabajo de producto nuevo (#38 en adelante).
