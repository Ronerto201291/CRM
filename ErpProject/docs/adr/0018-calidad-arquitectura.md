...
| 55 | **Desaparcado** — Postgres publicado a `0.0.0.0:5432` sin bloqueo de firewall. Ya no es solo una hipótesis "sin servidor": `docker-compose.prod.yml` existe y está listo para usarse en un servidor real (ADR-0020), y ese override **no toca el servicio `postgres`** — el `ports: "5432:5432"` de la base sigue activo también en el stack de producción, verificado con `docker compose -f docker-compose.yml -f docker-compose.prod.yml config`. El día que haya un servidor con IP pública, la base de datos quedaría accesible desde fuera con solo la contraseña. Corregir antes de cualquier despliegue real: quitar el `ports:` de `postgres` en `docker-compose.prod.yml` (no hace falta publicarlo si backend/postgres están en la misma red de Docker) | `docker-compose.yml:16-17`, `docker-compose.prod.yml` | **Alta — bloqueante antes de desplegar en un servidor real** |
